...
### file%3A%2F%2F%2Fhome%2Fandresuv%2FcuartoSemestre%2FPFC%2Fproyecto%2Fproyectocode%2Fproyectofinal%2Fsrc%2Ftest%2Fscala%2FPruevasPar.worksheet.sc:266: error: `)` expected but `}` found
        } else {
        ^
    ...
    if (cod1 == cod2) // Si se llega al destino, devolver una lista vacía
      ...
      val result=vuelosDesdeCod1.flatMap { vuelo =>
        if (!visitados(vuelo.Dst)) {
          ...
          generarItinerarios(vuelo.Dst, cod2, visitados + vuelo.Dst)
          itinerariosRestantes.map(vuelo :: _)},vuelosDesdeCod1.length
