...
### file%3A%2F%2F%2Fhome%2Fandresuv%2FcuartoSemestre%2FPFC%2Fproyecto%2Fproyectocode%2Fproyectofinal%2Fsrc%2Ftest%2Fscala%2Fpruebas.worksheet.sc:78: error: `identifier` expected but `else` found
else{
^
...
if (vuelos.length==0) List(vuelo):
else{
    if(vuelos.head.Dst==vuelo.Org) vuelos.head::formarItinerario(vuelo,vuelos.tail)
}
}
